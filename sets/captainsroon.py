...
k = 5
list_room = [1, 2, 3, 6, 5, 4, 4, 2, 5, 3, 6, 1, 6, 5,
             3, 2, 4, 1, 2, 5, 1, 4, 3, 6, 8, 4, 3, 1, 5, 6, 2]

# k = int(input())

# list_room = list(map(int,input().split(' ')))
#lento para listas longas
for i in set(list_room):
    if list_room.count(i) == 1:
        print(i)
   


# forma mais rapida para lista maiores
contagem_ocorrencias = {}
for elemento in list_room:
    contagem_ocorrencias[elemento] = contagem_ocorrencias.get(elemento,0) + 1
print(contagem_ocorrencias)

for elemento, contagem in contagem_ocorrencias.items():
    if contagem == 1:
        print(elemento)
